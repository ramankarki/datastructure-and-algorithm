...
function sumArray(array) {
  ...
  return sumArray([array[0] + array[1], ...array.slice(2)]);
}
...
function pow(base, exponent) {
  ...
  if (exponent < 0) {
    exponent *= -1;
    return 1 / (pow(base, exponent - 1) * base);
  }
...
function fileFinder(directories, targetFile) {
  for (let i in directories) {
    if (i === targetFile) return true;
    else if (i.startsWith("/")) {
      if (fileFinder(directories[i], targetFile)) return true;
    }
  }
...
function pathFinder(directories, targetFile) {
  for (let i in directories) {
    if (i === targetFile) return targetFile;
    else if (i.startsWith("/")) {
      let dir = pathFinder(directories[i], targetFile);
      if (dir === targetFile) return i + "/" + targetFile;
      else if (dir && dir.endsWith(targetFile)) return i + dir;
    }
  }
