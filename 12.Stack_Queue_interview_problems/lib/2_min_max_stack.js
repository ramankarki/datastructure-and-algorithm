...
class MinMaxStack {
  ...
  push(val) {
    ...
    } else if (newNode.value >= this.maxStack[this.maxStack.length - 1].value) {
      this.maxStack.push(newNode);
    }
    // to here
  ...
  pop() {
    ...
    if (temp === this.maxStack[this.maxStack.length - 1]) {
      this.maxStack.pop();
    }
    // to here
...
// Forgetting something down here? => MinMaxStack
exports.Node = Node;
